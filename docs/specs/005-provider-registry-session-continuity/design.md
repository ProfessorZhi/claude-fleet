...
| `minimax`                        | anthropic-compatible   | auth-token                | 中国默认 `https://api.minimaxi.com/anthropic`；国际用户可改为 `https://api.minimax.io/anthropic` | `ANTHROPIC_AUTH_TOKEN`（用户 Key）；`CLAUDE_CODE_AUTO_COMPACT_WINDOW=1000000`（1M 上下文）；官方建议 `ANTHROPIC_MODEL=MiniMax-M3[1m]` + `ANTHROPIC_DEFAULT_*_MODEL`                                                                                                                               | `MiniMax-M3[1m]`                           |
