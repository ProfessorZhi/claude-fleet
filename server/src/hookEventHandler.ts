...
export class HookEventHandler {
  ...
  constructor(
    ...
    if (provider.protocolVersion !== HookEventHandler.SUPPORTED_PROTOCOL_VERSION) {
      console.warn(
        `[Claude Fleet] HookProvider "${provider.id}" reports protocolVersion=${provider.protocolVersion}, ` +
  ...
  registerAgent(sessionId: string, agentId: number): void {
    ...
    if (debug && flushed.length > 0)
      console.log(
        `[Claude Fleet] Hook: flushing ${flushed.length} buffered event(s) for session ${sessionId.slice(0, 8)}...`,
      );
  ...
  handleEvent(_providerId: string, event: HookEvent): void {
    ...
    if (normEvent.kind === 'sessionStart') {
      ...
      if (debug && tracked)
        console.log(`[Claude Fleet] Hook: SessionStart(source=${source}, session=${sid}...)`);
      ...
      if (existingAgentId !== undefined) {
        ...
        if (debug)
          console.log(
            `[Claude Fleet] Hook: Agent ${existingAgentId} - SessionStart(source=${source}) known`,
          );
        return;
      }
        ...
        if (projectDir) {
          for (const [id, agent] of this.agents) {
            ...
            if (isMatch) {
              ...
              console.log(
                `[Claude Fleet] Hook: Agent ${id} - /${normEvent.source} detected, reassigning to ${event.session_id}`,
              );
      ...
      if (transcriptPath || cwd) {
        ...
        if (debug && tracked)
          console.log(
            `[Claude Fleet] Hook: SessionStart(source=${source}) -> pending external session ${sid}..., awaiting confirmation`,
          );
        ...
        if (debug && tracked)
          console.log(
            `[Claude Fleet] Hook: SessionStart -> unknown session ${sid}..., no transcript_path`,
          );
      }
      return;
    }
    ...
    if (normEvent.kind === 'sessionEnd' && this.sessionRouter.hasPending(event.session_id)) {
      ...
      if (debug)
        console.log(
          `[Claude Fleet] Hook: SessionEnd discarded pending external session ${event.session_id.slice(0, 8)}...`,
        );
      return;
    }
    ...
    if (pending) {
      if (debug)
        console.log(
          `[Claude Fleet] Hook: ${eventName} confirmed external session ${event.session_id.slice(0, 8)}..., notifying host`,
        );
    ...
    if (agentId === undefined) {
      ...
      if (isPending || hasBuffered || hasUnregisteredAgents) {
        if (debug)
          console.log(
            `[Claude Fleet] Hook: ${eventName} - unknown session ${event.session_id.slice(0, 8)}..., buffering`,
          );
    ...
    if (debug)
      console.log(
        `[Claude Fleet] Hook: Agent ${agentId} - ${eventName} (session=${event.session_id.slice(0, 8)}...)`,
      );
  ...
  private handleSessionEnd(
    ...
    if (debug)
      console.log(
        `[Claude Fleet] Hook: Agent ${agentId} - SessionEnd(reason=${reason ?? 'unknown'})`,
      );
    ...
    if (expectsFollowUp) {
      ...
      if (debug)
        console.log(
          `[Claude Fleet] Hook: Agent ${agentId} - SessionEnd(reason=${reason}), awaiting possible SessionStart`,
        );
  ...
  private handleSubagentStart(event: HookEvent, agent: AgentState, agentId: number): void {
    ...
    if (this.provider.team && agent.currentHookIsTeammateSpawn === true && agent.teamName) {
      if (debug)
        console.log(
          `[Claude Fleet] Hook: Agent ${agentId} - SubagentStart: teammate "${agentType}" detected, triggering discovery`,
        );
  ...
  private handleSubagentStop(agent: AgentState, agentId: number): void {
    ...
    if (inlineTeammates.length > 0) {
      if (debug)
        console.log(
          `[Claude Fleet] Hook: Agent ${agentId} - SubagentStop: marking inline teammates as waiting`,
        );
  ...
  private handleTeammateIdle(event: HookEvent, agent: AgentState, agentId: number): void {
    ...
    if (teammateName) {
      ...
      if (match) {
        ...
        if (debug)
          console.log(
            `[Claude Fleet] Hook: TeammateIdle "${teammateName}" -> teammate Agent ${id}`,
          );
        this.markAgentWaiting(a, id);
        return;
      }
    ...
    if (debug)
      console.log(
        `[Claude Fleet] Hook: TeammateIdle (no teammate_name match) -> marking ${inlineTeammates.length} teammate(s) done`,
      );
    for (const [id, a] of inlineTeammates) {
      this.markAgentWaiting(a, id);
    }
  ...
  private handleTaskCompleted(event: HookEvent, agentId: number): void {
    ...
    if (debug)
      console.log(
        `[Claude Fleet] Hook: Agent ${agentId} - TaskCompleted: ${taskSubject}${teammateName ? ` (teammate_name=${teammateName})` : ''}`,
      );
